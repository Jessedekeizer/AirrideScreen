//* LVGL 9.5 config for Cheap Yellow Display (ESP32-2432S028R)

//* LOCATION: project include/ folder — resolved via -DLV_CONF_INCLUDE_SIMPLE and -I./include/ in platformio.ini
...
/** Enable log module */
/* Off: the log calls sit inside the draw paths and cost frames. See the
 * performance table in CLAUDE.md. Turning this to 1 also needs my_print() in
 * src/main.cpp, which is already there behind #if LV_USE_LOG.
 *
 * Note this silences LVGL's own out-of-memory warning, so an allocation that
 * fails does so quietly and the object simply reads back as null. If a crash
 * ever looks like a null widget, put this back to 1 with LV_USE_ASSERT_OBJ and
 * LV_USE_ASSERT_MALLOC below - that is what found the draw buffer sizing. */
#ifndef LV_USE_LOG
#define LV_USE_LOG 0
#endif
#if LV_USE_LOG
...
/* The two slow ones are off. Both are debugging measures: turn them on with
 * LV_USE_LOG to put a name on a crash, then turn them back off. */
